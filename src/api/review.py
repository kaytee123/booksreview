...
class Review:
    def __init__(self, review_id: str, name: str, rating: int, review: str, created_at: datetime):
        self.id = review_id
        self.name = name
        self.rating = rating
        self.review = review
    ...
    def validate(book):
        ...
        if not book.get('rating') or book.get('rating') > 5 or book.get('rating') < 0:
...
class ReviewListMapper:
    ...
    def to_dict(review_list: ReviewList):
        reviews = [ReviewMapper.to_dict(review)
    ...
    def to_persistence(review_list: ReviewList):
        def map_review(review): return ReviewMapper.to_dict(
            review, id_shape='_id')
